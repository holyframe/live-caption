@echo off
REM Builds and runs the caption latency diagnostic.
setlocal
...
cl /nologo /std:c++20 /W4 /permissive- /utf-8 /EHsc /MT /O2 ^
   ...
   /I src ^
   tests\latency_probe.cpp src\CaptionSource.cpp src\Util.cpp ^
   /Fo"%OBJDIR%\\" /Fe"build\latency_probe.exe" ^
   /link user32.lib ole32.lib oleaut32.lib uuid.lib
if errorlevel 1 (
    popd
    exit /b 1
)
